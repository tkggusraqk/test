          ...
          <ul>
            <li class="condition_li" v-for="(item,index) in circleData" :key="item.id">
              ...
              <div class="condition_right">
                ...
                <div class="publishtext" style="text-align:left;">
                  {{item.content}}
                </div>
                ...
                <div class="commentbutton">
                  <div class="button_left clear">
                    ...
                    <span v-if="userInfo.id == item.user_id ? true : false" @click="delItem(index)">删除</span>
                  </div>
                ...
                <div class="retext" v-show="item.likeList.length >0 || item.recent_replies.length > 0">
                  ...
                  <div class="retext_revert" v-show="item.recent_replies.length > 0">
                    <ul>
                      <li v-for="value in item.recent_replies" :key="value">
                        <span>{{value.name}}</span>：{{value.content}}
                      </li>
                    </ul>
                  </div>
                </div>
  ...
  methods: {
    ...
    getLikeList(index, id) {
      $.post(this.apiUrl + 'social/like-list/?token=' + utils.token, { id: id }, function (res) {
        if (res.code === 200 && res.data.length > 0) {
          this.circleData[index].likeList = res.data
        }
      }.bind(this))
    },
    exportInput() {
    ...
    supportThing(item) {//点赞
      ...
      if (item.suporthtml == "赞") {
        item.suporthtml = "取消";
        item.like.push(this.userInfo.name)
      } else {
        item.suporthtml = "赞";
        item.like.pop();
      }
      ...
      if (this.changeinput) {
        if (this.textareaVlue) {
          this.itemlist.comment.push({
            wxid: this.userInfo.id,
            petname: this.userInfo.name,
